    ...
    while true do
		...
		if IsControlJustPressed(1, 243) then
            ...
            if voice.current == 0 then
				...
				TriggerEvent("notification", "#3333FF", "STIMMENLAGE", "Normal (6 Meter)")
            elseif voice.current == 1 then
                NetworkSetTalkerProximity(voice.shout)
                SendNUIMessage({action = "setVoiceLevel", level = 3});
				voiceCoords = voice.shout
				TriggerEvent("notification", "#ab0503", "STIMMENLAGE", "Schreien (15 Meter)")
            elseif voice.current == 2 then
                NetworkSetTalkerProximity(voice.whisper)
                SendNUIMessage({action = "setVoiceLevel", level = 1});
				voiceCoords = voice.whisper
				TriggerEvent("notification", "#E4E652", "STIMMENLAGE", "Flüstern (2 Meter)")
            end
