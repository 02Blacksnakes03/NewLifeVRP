...
function LoadJailCell(timu, firstTime)
	...
	ESX.TriggerServerCallback('HD_Jail:GrabInfoLoc', function(locnum)
		infoLoc = locnum
	end)
	...
	Citizen.CreateThread(function()
		ESX.TriggerServerCallback('HD_Jail:GetCell', function(cell)
			if cell ~= 0 then
...
function OpenBreakingMenu()
	...
	Citizen.CreateThread(function()
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fence_menu', {
			...
			if flip then
				ESX.TriggerServerCallback('HD_Jail:CheckItemB2', function(can)
					if can then
						menu.close()
						local hnum = 0
...
function OpenWallMenu()
	...
	Citizen.CreateThread(function()
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'wall_menu', {
			...
			ESX.TriggerServerCallback('HD_Jail:CheckItemB', function(can)
				if can then
					local hnum = 0
					local rannum = math.random(1,10)
					hnum = rannum
...
function OpenPoliceShitMenu()
	using = true
	ESX.TriggerServerCallback('HD_Jail:GetPlayerInCell', function(players)
		local ped = PlayerPedId()
		local element = {}
...
function OpenPoliceMenu2(name, theirID)
	Citizen.CreateThread(function()
		...
		ESX.TriggerServerCallback('HD_Jail:GetChest2', function(cvalue)
			local valuesc = {}
			local element = {}
	...
	Citizen.CreateThread(function()
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'make_menu2', {
			...
			if data.current.value == 'need' then
				...
				menu.close()
				ESX.TriggerServerCallback('HD_Jail:CheckItemMake', function(can)
					if can == 1 then
...
function OpenAddChest()
	local ped = PlayerPedId()
	ESX.TriggerServerCallback('HD_Jail:GetInventory', function(inv)
		local invie = nil
		local element = {}
...
function OpenItChest()
	local ped = PlayerPedId()
	ESX.TriggerServerCallback('HD_Jail:GetChest', function(cvalue)
		local valuesc = {}
		local element = {}
...
AddEventHandler('HD_Jail:JailMenu', function()
	ESX.TriggerServerCallback('HD_Jail:CheckLockdown', function(elemento)
		local element = {}
		element = elemento
				...
				if police then
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'id_amt', {
						...
						if amount == nil or amount < 0 then
							...
							ESX.TriggerServerCallback('HD_Jail:CheckID', function(can)
								if can then
				...
				if police then
					...
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'id_amt', {
						...
						if amount == nil or amount < 0 then
							...
							ESX.TriggerServerCallback('HD_Jail:CheckID2', function(can)
								if can then
									ESX.UI.Menu.CloseAll()
		...
		if data.current.value == 'id' then
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'id_amt2', {
				...
				if amount == nil or amount < 0 then
					...
					ESX.TriggerServerCallback('HD_Jail:CheckID2', function(yurrrr)
						if yurrrr then
							menu.close()
