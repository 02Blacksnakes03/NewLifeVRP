    ...
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
        ...
        for key, vehicleData in ipairs(fetchedVehicles) do
            ...
            table.insert(menuElements, {
                ["label"] = "Ausparken: " .. GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " Bestellnummer: " .. vehicleData["plate"],
                ["vehicle"] = vehicleData
            })
        end
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "Du hast dein Auto nicht hier eingeparkt!"
            })
...
OpenVehicleMenu = function()
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
            ...
            for _, vehicle in ipairs(gameVehicles) do
                if DoesEntityExist(vehicle) then
                    ...
                    if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                        table.insert(menuElements, {
                            ["label"] = GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " mit dem Kennzeichen - " .. vehicleData["plate"] .. " - " .. dstCheck .. " ist zu weit weg!",
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "Du besitzt kein Fahrzeug!"
            })
        end
...
ChooseVehicleAction = function(vehicleEntity, callback)
    ...
    local menuElements = {
        {
            ["label"] = (GetVehicleDoorLockStatus(vehicleEntity) == 1 and "Geschlossen" or "Öffnen") .. " vehicle.",
...
VehicleAction = function(vehicleEntity, action)
    ...
    elseif action == "change_door_state" then
        if dstCheck >= Config.RangeCheck then return ESX.ShowNotification("Túl messze vagy a kocsidtól, hogy vezéreld.") end
...
SpawnVehicle = function(vehicleProps)
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        ESX.ShowNotification("~b~[Information]:~s~ Du hast das Fahrzeug erfolgreich ausgeparkt!")
	...
	if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("garage:validateVehicle", function(valid)
			if valid then
				...
				ESX.ShowNotification("~b~[Information]:~s~ Du hast das Fahrzeug erfolgreich eingeparkt!")
			else
				ESX.ShowNotification("~r~[Information]:~s~ Bist du sicher das dass Fahrzeug dir gehört?")
			end
...
WaitForModel = function(model)
	...
	while not HasModelLoaded(model) do
		...
		DrawScreenText("Kocsi betöltése " .. GetLabelText(GetDisplayNameFromVehicleModel(model)) .. "...", 255, 255, 255, 150)
	end
end