...
function StopTheoryTest(success)
	...
	if success then
		...
		notify(_U('passed_test'))
	else
		notify(_U('failed_test'))
	end
end
...
function OpenDMVSchoolMenu()
	...
	if ownedLicenses['dmv'] then
		...
		if not ownedLicenses['drive_truck'] then
			...
			SendNUIMessage({
				openSection = 'box-3-lock'
			})
		end
	end
	if ownedLicenses['dmv'] then
	SendNUIMessage({
		openSection = 'openMainMenu'
	})
	SetNuiFocus(true, true)
	end
end

RegisterNUICallback("kilit", function ()
	girdimi = false
	end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, Keys['E']) then
				print(CurrentAction , girdimi)
				if CurrentAction == 'dmvschool_menu' then
					if girdimi then
						print("kaka")
					TriggerServerEvent("esx_dmvschool:get")
					OpenDMVSchoolMenu()
					else
						notify(_U("replay"))	
				end
	...
	AddEventHandler("school:cl:get")
	RegisterNetEvent("school:cl:get", function (cash,name)
		SendNUIMessage({
			type = "update",
			cash = cash,
			name = name
	...
	while true do
		...
		if CurrentTest == 'drive' then
			...
			if Config.CheckPoints[nextCheckPoint] == nil then
				...
				notify(_U('driving_test_complete'))
				...
				for k,v in pairs(Config.SpeedLimits) do
					if CurrentZoneType == k and speed > v then
						...
						if not IsAboveSpeedLimit then
							...
							notify(_U('driving_too_fast', v))
							notify(_U('errors', DriveErrors, Config.MaxErrors))
						end
					end
				end
...
RegisterNUICallback("drive", function ()
	SetNuiFocus(false, false)	
	StartDriveTest("drive")

end)

RegisterNUICallback("drive2", function ()
	SetNuiFocus(false, false)	
	StartDriveTest("drive_bike")
end)

RegisterNUICallback("drive3", function ()
	SetNuiFocus(false, false)	
...
--   function StartDriveTest(type)
-- 	  QBCore.Functions.SpawnVehicle(Config.VehicleModels[type], Config.Zones.VehicleSpawnPoint.Pos, Config.Zones.VehicleSpawnPoint.Pos.h, function(vehicle)
-- 		  CurrentTest       = 'drive'
-- 		  CurrentTestType   = type
-- 		  CurrentCheckPoint = 0
-- 		  LastCheckPoint    = -1
