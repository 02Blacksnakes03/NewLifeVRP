    ...
    if hasJob then
        ...
        for k, v in pairs(Vehicles) do 
            if v.storage > 0 then
                ...
                price_item:RightLabel('~HUD_COLOUR_GREENDARK~$' .. v.price)
                model_item:RightLabel(v.label)
                storage:RightLabel('~b~' .. v.storage)
...
function GenerateComputerMenu(x, y, z, rot)
    ...
    for k,v in pairs(Vehicles) do
        ...
        if isBoss then
            ...
            produce:RightLabel('~HUD_COLOUR_GREENDARK~$' .. math.floor(v.price*0.75))
    ...
    if isBoss then
        ...
        for k,v in pairs(Vehicles) do
            if v.produced == 1 then
                ...
                time2 = NativeUI.CreateItem(Translation[Cfg.Locale]['total_production_time'], '')
...
function GeneratePlate()
	...
	while true do
		...
		ESX.TriggerServerCallback('myCardealer:isPlateTaken', function(isPlateTaken)
