...
ESX.RegisterServerCallback('myCardealer:isPlateTaken', function(source, cb, plate)
...
AddEventHandler('myCardealer:CheckTime', function(time, lastorder, model, oldstorage, total)
    ...
    elseif hours > time then
        MySQL.Async.execute(
            'UPDATE cardealer SET storage = @storage, produced = @produced, producetotal = @producetotal WHERE model = @model', {
                ['@storage'] = newStorage, 
                ['@model'] = model, 
                ['@produced'] = false, 
                ['@producetotal'] = 0, 
