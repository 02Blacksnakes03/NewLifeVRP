...
AddEventHandler('esx_identity:setIdentity', function(data, myIdentifiers)
	setIdentity(myIdentifiers.steamid, data, function(callback)
		if callback then
			...
			TriggerClientEvent('chat:addMessage', source, { args = { '^[IDENTITY]', 'Fehler beim Setzen des Zeichens, versuchen Sie es später erneut oder wenden Sie sich an den Serveradministrator!' } })
		end
	end)
end)
...
TriggerEvent('es:addCommand', 'register', function(source, args, user)
	getCharacters(source, function(data)
		if data.firstname3 ~= '' then
			TriggerClientEvent('chat:addMessage', source, { args = { '^[IDENTITY]', 'Sie können nur 3 registrierte Zeichen haben. Verwenden Sie den Befehl ^ 3 / chardel ^ 0, um vorhandene Zeichen zu löschen.' } })
		else
			TriggerClientEvent('esx_identity:showRegisterIdentity', source, {})
		end
	end)
end, {help = "Registrieren Sie ein neues Charakter"})

TriggerEvent('es:addGroupCommand', 'char', 'user', function(source, args, user)
	getIdentity(source, function(data)
		if data.firstname == '' then
			TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Du hast keinen aktiven Charakter!' } })
		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Aktiver Charakter: ^2' .. data.firstname .. ' ' .. data.lastname } })
		end
	end)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Nicht ausreichende Berechtigungen!' } })
end, {help = "Listen Sie Ihren aktuellen Charakter auf"})

TriggerEvent('es:addGroupCommand', 'charlist', 'user', function(source, args, user)
	getCharacters(source, function(data)
		if data.firstname1 ~= '' then
			TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY] Charakter 1:', data.firstname1 .. ' ' .. data.lastname1 } })
			
			if data.firstname2 ~= '' then
				TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY] Charakter 2:', data.firstname2 .. ' ' .. data.lastname2 } })
				
				if data.firstname3 ~= '' then
					TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY] Charakter 3:', data.firstname3 .. ' ' .. data.lastname3 } })
				end
			end
		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^[IDENTITY]', 'Sie haben keine registrierten Zeichen. Verwenden Sie den Befehl ^ 3 / register ^ 0, um ein Zeichen zu registrieren.' } })
		end
	end)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Nicht ausreichende Berechtigungen!' } })
end, {help = "Listen Sie alle Ihre registrierten Charakterauf"})
...
TriggerEvent('es:addGroupCommand', 'charselect', 'user', function(source, args, user)
	...
	if charNumber == nil or charNumber > 3 or charNumber < 1 then
		TriggerClientEvent('chat:addMessage', source, { args = { '^[IDENTITY]', 'Das ist ein ungültiger Charakter!' } })
		return
	end
		...
		if charNumber == 1 then
			...
			if data.firstname ~= '' then
				updateIdentity(GetPlayerIdentifiers(source)[1], data, function(callback)
					if callback then
						TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Aktualisierte deinen aktiven Charakter auf ^2' .. data.firstname .. ' ' .. data.lastname } })
					else
						TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Fehler beim Aktualisieren Ihrer Identität, versuchen Sie es später erneut oder wenden Sie sich an den Serveradministrator!' } })
					end
				end)
			else
				TriggerClientEvent('chat:addMessage', source, { args = { '^1[IDENTITY]', 'Du hast keinen Charakter in Slot 1!' } })
			end
