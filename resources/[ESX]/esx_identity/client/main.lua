...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		TriggerEvent('chat:addMessage', { args = { '^1[IDENTITY]', '^1Sie müssen Ihren ersten Charakter erstellen, um spielen zu können' } })
	end
end)
...
RegisterNUICallback('register', function(data, cb)
	...
	for theData, value in pairs(myIdentity) do
		if theData == "Vorname" or theData == "Nachname" then
		...
		elseif theData == "height" then
			...
			if height then
				if height > 200 or height < 140 then
					reason = "Inakzeptable Spielergröße!"
					break
				end
			else
				reason = "Inakzeptable Spielergröße!"
...
function verifyName(name)
	...
	if nameLength > 25 or nameLength < 2 then
		return 'Ihr Spielername ist entweder zu kurz oder zu lang.'
	end
	...
	if count ~= nameLength then
		return 'Ihr Spielername enthält Sonderzeichen, die auf diesem Server nicht zulässig sind.'
	end
	...
	if spacesInName > 2 then
		return 'Ihr Name enthält mehr als zwei Leerzeichen'
	end
	
	if spacesWithUpper ~= spacesInName then
		return 'Ihr Name muss mit einem Großbuchstaben beginnen.'
	end
