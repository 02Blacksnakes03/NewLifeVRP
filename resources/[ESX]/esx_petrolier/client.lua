    ...
    while true do
        ...
        if faza1 == true then
            if metri2 <=4 then
                ...
                if not text then
                    DrawText3D(2926.9167480469,4301.8979492188,50.434585571289, "PDrücke [~g~E~w~] um den LKW zu beladen", 1.2)
                    ...
                    if IsControlJustPressed(1,51) then
                        if (Vdist(pos.x, pos.y, pos.z, carPos.x , carPos.y, carPos.z) <= 50.0) and hasJob and faza1 and DoesEntityExist(veh) then
                            if incarcat == false then   
                                ...
                                faza2 = true
                                faza1 = false
                                exports['progressBars']:startUI(20000, "LKW wird beladen...")
                                Wait(20000) 
                                ESX.ShowNotification('Sie haben den LKW beladen,und können fahren.')
                                ...
                                AddTextComponentString("Kraftstoff: Öl liefern")
                                EndTextCommandSetBlipName(blip)
                                table.insert(Ruote, blip)

                                FreezeEntityPosition(veh,false)
                            else
                                ESX.ShowNotification('Sie haben den LKW bereits beladen')
                            end
                        else
                ...
                elseif text == true then
                    DrawText3D(2926.9167480469,4301.8979492188,50.434585571289, "~y~Liefer das ÖL", 1.2)
                end
            end
        end
        if faza2 then
            if metri3 <=4 then
                ...
                DrawText3D(1372.0041503906,3623.0671386719,34.87410736084, "Drücke[~g~E~w~] ~w~um den LKW zu entladen.", 1.2)
                ...
                if IsControlJustPressed(1,51) then         
                    if (Vdist(pos.x, pos.y, pos.z, carPos.x , carPos.y, carPos.z) <= 50.0) and hasJob and faza2 and DoesEntityExist(veh) then 
                        FreezeEntityPosition(veh,true)
                        exports['progressBars']:startUI(20000, "LKW wird entladen...")
                        Wait(20000) 
                        cursaterminata = true
        ...
        if metri <=2 and PlayerData.job ~= nil and (PlayerData.job.name == 'petrolier' or PlayerData.org.name == 'petrolier') then
            DrawText3D(pos.x,pos.y,pos.z+0.3, "Drücke [~g~E~w~] um den Lieferanten Job zu starten", 1.2)
            if IsControlJustPressed(1,51) then
                if hasJob == false then
                    ...
                    ESX.ShowNotification('Du machst diesen Job schon')
                end
            end
        end
    end
...
function refreshBlips()
    if PlayerData.job ~= nil and (PlayerData.job.name == 'petrolier' or PlayerData.org.name == 'petrolier') then
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString("Fueler - Job starten")
		EndTextCommandSetBlipName(blip)
