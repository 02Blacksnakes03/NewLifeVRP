            ...
            if dist <= 0.5 then
				...
				if IsControlJustPressed(0, Keys['E']) then -- "E"
					if IsPedInAnyVehicle(GetPlayerPed(-1)) then
						ESX.ShowNotification("Du hast schon ein ~r~Fahrzeug")
					else
						OpenBikeMenu()
					end
				end			
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(gym) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Drücke ~INPUT_CONTEXT~ um das ~b~GYM~w~ Menü zu öffnen')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(arms) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Drücke ~INPUT_CONTEXT~ um deine ~g~Arme~s~ zu trainieren.')
				
				if IsControlJustPressed(0, Keys['E']) then
					if training == false then
					
						TriggerServerEvent('esx_gym:checkChip')
						ESX.ShowNotification("Bereitet die ~g~Übung~s~ vor...")
						Citizen.Wait(1000)					
						...
						if membership == true then
							...
							ESX.ShowNotification("Du musst ~r~60 Sekunden~w~ warten bevor du die nächste Übung machen kannst.")
							
							--TriggerServerEvent('esx_gym:trainArms') ## COMING SOON...
							
							training = true
						elseif membership == false then
							ESX.ShowNotification("Du brauchst eine ~b~GYM Mitgliedschaft~s~ um hier zu trainieren.")
						end
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(chins) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Drücke ~INPUT_CONTEXT~ um ein paar ~g~pull-ups~s~ zu machen')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(pushup) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Drücke ~INPUT_CONTEXT~ um ~g~pushups~s~ zu machen.')
...
Citizen.CreateThread(function()
    while true do
        ...
        for k in pairs(yoga) do
            ...
            if dist <= 0.5 then
				hintToDisplay('Drücke ~INPUT_CONTEXT~ um ~g~Yoga~s~ zu machen.')
...
function CheckTraining()
	if resting == true then
		ESX.ShowNotification("Du musst eine Pause machen...")
	...
	if resting == false then
		ESX.ShowNotification("Du bist jetzt ausgeruht...")
	end
end
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'gym_menu',
        {
            ...
            elements = {
				...
				{label = 'Öffnungszeiten', value = 'hours'},
				{label = 'Mitgliedschaft', value = 'ship'},
            }
        },
        function(data, menu)
            ...
            elseif data.current.value == 'hours' then
				...
				ESX.ShowNotification("Wir haben ~g~24/7~w~ geöffnet. Willkommen.")
