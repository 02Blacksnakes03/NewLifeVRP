...
AddEventHandler('esx_gym:hireBmx', function()
	...
	if(xPlayer.getMoney() >= 250) then
		...
		notification("Du hast ein ~g~BMX~s~ gemietet")
	else
		notification("Du hast das BMX gestohlen weil du nicht genug ~r~Geld~s~ hast")
	end	
end)
...
AddEventHandler('esx_gym:hireCruiser', function()
	...
	if(xPlayer.getMoney() >= 300) then
		...
		notification("Du hast ein ~g~Fahrrad~s~ gemietet")
	else
		notification("Du hast das Fahrrad gestohlen weil du nicht genug ~r~Geld~s~ hast")
	end	
end)
...
AddEventHandler('esx_gym:buyMembership', function()
	...
	if(xPlayer.getMoney() >= 800) then
		...
		notification("Du hast eine ~g~GYM Mitgliedschaft~s~ erworben.")
...
AddEventHandler('esx_gym:buyProteinshake', function()
	...
	if(xPlayer.getMoney() >= 6) then
		...
		notification("Du hast ein ~g~Protein shake~s~ gekauft.")
	else
		notification("Du hast nicht genung ~r~Geld")
	end	
end)
...
ESX.RegisterUsableItem('protein_shake', function(source)
	...
	TriggerClientEvent('esx:showNotification', source, 'Du hast einen ~g~Protein shake~s~ getrunken')
...
AddEventHandler('esx_gym:buyWater', function()
	...
	if(xPlayer.getMoney() >= 1) then
		...
		notification("Du hast ein ~g~Wasser~s~ gekauft.")
	else
		notification("Du hast nicht genug ~r~Geld")
	end		
end)
...
AddEventHandler('esx_gym:buySportlunch', function()
	...
	if(xPlayer.getMoney() >= 2) then
		...
		notification("Du hast ein ~g~Sportlunch~s~ gekauft.")
	else
		notification("Du hast nicht genug ~r~Geld")
	end		
end)
...
ESX.RegisterUsableItem('sportlunch', function(source)
	...
	TriggerClientEvent('esx:showNotification', source, 'Du hast ein ~g~Sportlunch~s~ gegessen.')
...
AddEventHandler('esx_gym:buyPowerade', function()
	...
	if(xPlayer.getMoney() >= 4) then
		...
		notification("Du hast einen ~g~Powerade~s~ gekauft.")
	else
		notification("Du hast nicht genug ~r~Geld")
	end		
end)
...
ESX.RegisterUsableItem('powerade', function(source)
	...
	TriggerClientEvent('esx_basicneeds:onDrink', source)
	TriggerClientEvent('esx:showNotification', source, 'Du hast einen ~g~Powerade~s~ getrunken.')
